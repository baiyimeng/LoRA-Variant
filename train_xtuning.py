...
from xpeft import MoeLoraConfig, PLoraConfig, TaskType, get_peft_model
...
def parse_arguments():
    ...
    parser.add_argument(
        "--peft_type", type=str, choices=["moelora", "plora"], default="plora"
    )
...
def train(args):
    ...
    if args.peft_type == "moelora":
        output_dir = os.path.join(
            ...
            args.model_name,
            args.peft_type + f"_{args.sample_ratio}_{args.num_experts}",
        )
    elif args.peft_type == "plora":
        output_dir = os.path.join(
            "/root/autodl-tmp/output",
